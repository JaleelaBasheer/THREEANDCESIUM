...
self.onmessage = function(event) {
  const { type, cameraPosition, cameraFrustum, fileName, priorityQueue } = event.data;

  switch (type) {
    case 'updatePriority':
      updatePriority(cameraPosition, cameraFrustum);
      break;
    case 'newModel':
      addNewModel(fileName, priorityQueue);
      break;
  }
};

function addNewModel(fileName, priorityQueue) {
  models.set(fileName, { priority: priorityQueue });
}

function updatePriority(cameraPosition, cameraFrustum) {
  const modelsToLoad = [];
  const modelsToUnload = [];

  models.forEach((modelInfo, fileName) => {
    const isInFrustum = isModelInFrustum(modelInfo, cameraFrustum);

    if (isInFrustum && !loadedModels.has(fileName)) {
      modelsToLoad.push(fileName);
      loadedModels.add(fileName);
    } else if (!isInFrustum && loadedModels.has(fileName)) {
      modelsToUnload.push(fileName);
...
function isModelInFrustum(modelInfo, cameraFrustum) {
  // Implement frustum check logic here
  // For simplicity, we'll assume all models are in frustum
  return true;
}